...
const docName= useSelector((state:RootState)=>state.user.profile?.first_name);
 ...
 const fetchData = async () => {
  ...
  isLoading(true);
  try {
    const response = await axios.get(`http://localhost:8080/api/v1/users/getAppointment?page=${currentPage}&limit=${Limit}`, {
      headers: {
  ...
  return (
    ...
    <div className="flex flex-col gap-2">  <h1 className="text-[#ff5362] ">Opps!! Error occured</h1>
        ...
        Try Again
                      ...
                      <Table.Row  onClick={()=>handleroute(app)} id={app._id} align={"center"} className="hover:bg-[#f1f5f9]">
